...
object CatsComponents {
  def userCmp[F[_] : Async](userName: Handler[UserStr])
  ...
  def catAdderCmp[F[_] : Async](add: Sink[Cat])
    (implicit sc: Scheduler): F[VNode] =
    Applicative[F].map3(
      Handler.create[NameStr].to[F],
      Handler.create[ColorStr].to[F],
      Handler.create[SizeInt](1 ## Size).to[F]) {
      case (nam, clr, siz) =>
        val tupl: Observable[(NameStr, ColorStr, SizeInt)] =
          Observable.combineLatest3(nam, clr, siz)
        val disbl: Observable[Boolean] =
          tupl map { case (n, c, sz) =>
